...
module.exports = function generateMedalChart(data, medalsvg) {
  ...
  medalsvg.append('g').attr('transform', "translate(100,200)");
  console.log("Medalsvg:", medalsvg); // ----- helper functions ----- //
  ...
  var xSmallAxisGroup = medalsvg.append("g").attr("class", "axis x").attr("transform", "translate(0," + innerHeight + ")").call(xSmallAxis);
  var x1 = d3.scaleBand();
  ...
  var slice = medalsvg.selectAll(".slice").data(groupData).enter().append("g").attr("class", "g").attr("transform", function (d) {
    return "translate(" + xSmallScale(d.key) + ",0)";
  });
  var currSelectedAthlete = null;
  slice.selectAll("circle").data(function (d) {
    return d.values;
  }).enter().append("circle").style("fill", function (d) {
    ...
    medalsvg.append("text").attr("id", "tooltip").attr("text-anchor", "middle").attr("transform", "translate(" + width / 2 + "," + innerHeight / 3 + ")").attr("font-family", "sans-serif").attr("font-size", "11px").attr("font-weight", "bold").attr("fill", "black").style("pointer-events", "none").text(d.grpAthlete);
    medalsvg.append("text").attr("id", "tooltip").attr("text-anchor", "middle").attr("transform", "translate(" + width / 2 + "," + (innerHeight / 3 + 20) + ")").attr("font-family", "sans-serif").attr("font-size", "11px").attr("font-weight", "bold").attr("fill", "black").style("pointer-events", "none").text(d.grpEvent);
  ...
  }).attr("cx", cxOffset("Silver") * xSmallScale.bandwidth()).attr("cy", 0).transition().delay(function (d) {
  ...
  .text("Medals Won"); // medalsvg.append("text")
...
function redrawMedals(slice, currSelectedAthlete) {
  ...
  slice.selectAll("circle") // .style("stroke", function(d) {
  //     return d.grpAthlete === currSelectedAthlete ? "black" : undefined;
  // })
...
function () {
  function rankRows(rowDiv, topCountryToRatio) {
    ...
    for (var i = 0; i < topCountryToRatio.length; i++) {
      ...
      d3.select("#row" + countryName).append("img").attr("src", "flags/" + imgcountryName + "-flag.svg").attr("id", "img" + countryName).attr("width", 90).attr("height", 60);
  ...
  _createClass(rankRows, [{
    ...
    value: function updateRankRowsSport(rowDiv, topCountryToRatio) {
      ...
      for (var i = 0; i < topCountryToRatio.length; i++) {
        ...
        d3.select("#row" + countryName).append("img").attr("src", "flags/" + imgcountryName + "-flag.svg").attr("id", "img" + countryName).attr("width", 90).attr("height", 60); // add text labels
...
function initializeDropdowns() {
  ...
  select.addEventListener('change', function () {
    ...
    rankRows.updateRankRowsSport(rankRowsDiv, topCountryToRatio); // update big chart
...
},{"d3":"UzF0","underscore":"h15N","./bigChart":"FthO","./rankRows":"V6gp"}]},{},["epB2"], null)
//# sourceMappingURL=https://uw-cse442-wi20.github.io/FP-olympians/main.a0a9b825.js.map