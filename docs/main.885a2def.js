...
},{}],"mtk6":[function(require,module,exports) {
var d3 = require('d3');

var _ = require("underscore"); // Draws the total bronze/silver/medals won by the given country
...
module.exports = function generateMedalChart(data, medalsvg) {
  console.log("generating medal counts chart!");
  console.log("DATA****", data);
  var minYear = 2000;
  var maxYear = 2020; // test

  medalsvg.append('g').attr('transform', "translate(100,200)"); // ----- helper functions ----- //

  var containsYear = function containsYear(groups, year) {
    return _.find(d3.values(groups), function (item) {
      return item.key === year;
    });
  };

  var getMedalCount = function getMedalCount(year, medal) {
    var numMedals = 0;
  ...
  var getEvents = function getEvents(year, medal) {
    var events = [];
    data.forEach(function (item) {
  ...
  var getAthletes = function getAthletes(year, medal) {
    var athletes = [];
    data.forEach(function (item) {
      // console.log("getting events for item:", item)
      item.values.forEach(function (athlete) {
  ...
  data.forEach(function (item) {
    ...
    var goldAthletes = getAthletes(year, 'Gold'); // console.log("item:", item)
    ...
    if (year >= minYear && year <= maxYear && containsYear(groupData, year) === undefined) {
      ...
      for (var i = 1; i <= bronze; i++) {
        medalMap.push({
          year: year,
          grpName: 'Bronze',
          grpValue: i,
          grpEvent: bronzeEvents[i - 1],
          grpAthlete: bronzeAthletes[i - 1]
        });
      }

      for (var _i = 1; _i <= silver; _i++) {
        medalMap.push({
          year: year,
          grpName: 'Silver',
          grpValue: _i,
          grpEvent: silverEvents[_i - 1],
          grpAthlete: silverAthletes[_i - 1]
        });
      }

      for (var _i2 = 1; _i2 <= gold; _i2++) {
        medalMap.push({
          year: year,
          grpName: 'Gold',
          grpValue: _i2,
          grpEvent: goldEvents[_i2 - 1],
          grpAthlete: goldAthletes[_i2 - 1]
        });
      }
  ...
  groupData.forEach(function (item) {
    item.values.forEach(function (row) {
      nodes.push(row);
    });
  });
  ...
  var getXDomain = function getXDomain() {
    var domain = [];

    for (var i = minYear; i <= maxYear; i += 4) {
      domain.push(i);
    }
  ...
  var xSmallScale = d3.scaleBand().domain(getXDomain()).range([margin.left, innerWidth]);
  ...
  var color = function color(medalType) {
    if (medalType === 'Bronze') return "#CD7F32";else if (medalType === 'Silver') return "#C0C0C0";else return "#D4AF37";
  };

  var cxOffset = function cxOffset(medalType) {
    if (medalType === 'Bronze') {
      return 0.3;
  ...
  var simulation = d3.forceSimulation(nodes) // .force('charge', d3.forceManyBody().strength(5))
  ...
  function boundingBox() {
    ...
    try {
      for (var _iterator = nodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        node.x = Math.max(Math.min(node.x, yearX + xSmallScale.bandwidth() / 2 - medalRadius), yearX - xSmallScale.bandwidth() / 2 + medalRadius); // console.log("max X = " + (yearX + xSmallScale.bandwidth() - medalRadius) + ", node X = " + node.x + ", min X = " + (yearX - xSmallScale.bandwidth() + medalRadius));            
  ...
  function ticked() {
    var u = medalsvg.selectAll('circle').data(nodes);
    u.enter().append('circle').attr('r', medalRadius).style('fill', function (d) {
      return color(d.grpName);
    }).merge(u).on("mouseover", function (d) {
      // Change tooltip text
      tooltipContainer.style("left", d3.mouse(this)[0] + "px").style("top", d3.mouse(this)[1] + document.getElementById('medalchart').getBoundingClientRect().top + medalRadius + "px").style("visibility", "visible").html("<p>" + d.grpAthlete + "<br>" + d.grpEvent + "</p>"); // highlight current circle selected
      ...
      return xSmallScale.bandwidth() * ((d.year - minYear) / 4 + 1);
    }).attr("cy", 0).transition().ease(d3.easeElastic).duration(function (d) {
...
},{"d3":"UzF0","underscore":"h15N"}],"FthO":[function(require,module,exports) {
...
function () {
  function bigChart() {
    ...
    this.width;
    this.height;
    this.margin = 50; // getting scale of graph
    // getting color sceme

    this.color = d3.scaleOrdinal(d3.schemeCategory10); // getting lineScale

    this.line; // getting the area where lines are added

    this.lines; // the svg containing the whole chart

    this.svg; // Additional data required for medalChart
  ...
  _createClass(bigChart, [{
    key: "drawChart",
    value: function drawChart(bigsvg, data, currSport, medalsvg, entriesBySportThenCountryThenYear) {
      var margin = {
        top: 20,
        right: 20,
        bottom: 30,
      ...
      this.height = parseInt(bigsvg.style("height"), 10); // this.width = this.width - margin.left - margin.right;
      // this.height = this.height - margin.top - margin.bottom;

      this.entriesBySportThenCountryThenYear = entriesBySportThenCountryThenYear;
      console.log("+++++++++++++++++++++++++");
      console.log(data);
      console.log("+++++++++++++++++++++++++"); // var width = 500;
      // var height = 300;
      // /* Format Data */
      ...
      xScale = d3.scaleLinear().domain([1980, 2016]).range([0, this.width - margin.left - margin.right - 30]);
      yScale = d3.scaleLinear().domain([0, 80]).range([this.height - margin.top - margin.bottom, 0]); // delete all lines

      /* Add SVG */

      svg = bigsvg.append("svg").attr("width", this.width + margin.left + margin.right).attr("height", this.height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.top + "," + margin.top + ")"); // this is just (30, 30 right now)
    ...
    key: "redraw",
    value: function redraw(bigsvg, data, currSport, medalsvg) {
      var _this = this;
      ...
      .style('stroke', function (d) {
        ...
        d3.select(this).style('opacity', lineOpacity).style("stroke-width", lineStroke);
...
},{"d3":"UzF0","underscore":"h15N","./medalChart":"mtk6"}],"V6gp":[function(require,module,exports) {
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
/*#__PURE__*/
function () {
  function rankRows(rowDiv, topCountryToRatio) {
    _classCallCheck(this, rankRows);
    ...
    for (var i = 0; i < topCountryToRatio.length; i++) {
      ...
      if (topCountryToRatio[i].value > 0) {
        ...
        countryName = topCountryToRatio[i].key.replace(/ /g, "");
        console.log(countryName); // mainDiv contains the right side of the rank item (the text)

        var mainDiv = this.topDivs[i].append("div").attr("width", "90px").attr("height", "40px").attr("id", "mainDiv" + countryName).style("padding", "5px").attr("display", "flex").attr("flex-direction", "column").attr("justify-content", "center").attr("align-content", "center");
        var nameDiv = mainDiv.append("div").attr("width", "90px").attr("height", "20px").attr("id", "country" + countryName).style("padding", "2px").append("text").attr("x", "40").attr("y", "10").style('color', 'White').style("font-size", "12px").style("font-weight", "bold").attr("text-anchor", "center") // centering doesn't work
  ...
  _createClass(rankRows, [{
    key: "roundRank",
    value: function roundRank(rank) {
    ...
    value: function updateRankRowsSport(rowDiv, topCountryToRatio) {
      ...
      for (var i = 0; i < topCountryToRatio.length; i++) {
        if (topCountryToRatio[i].value > 0) {
          ...
          var nameDiv = mainDiv.append("div").attr("width", "90px").attr("height", "20px").attr("id", "country" + countryName).style("padding", "2px").append("text").attr("x", "40").attr("y", "10").style('color', 'White').style("font-size", "12px").style("font-weight", "bold").attr("text-anchor", "center") // centering doesn't work
          .text(topCountryToRatio[i].key); // add ratio label

          var ratioDiv = mainDiv.append("div").attr("width", "90px").attr("height", "20px").attr("id", "ratio" + countryName).style("padding", "2px").append("text").attr("x", "40px").attr("y", "10px").style('color', "#000080").style("font-size", "12px").style("font-weight", "bold").attr("text-anchor", "center") // centering doesn't work
          .text(this.roundRank(topCountryToRatio[i].value)); // d3 transitions doesn't preserve css styling...
          // d3.select("#country" + i)
          //     .transition()
          //     .text(topCountryToRatio[i].key);
          //
    ...
    key: "updateRankRowsYear",
    value: function updateRankRowsYear(rowDiv, topCountryToRatio) {
      var oldswappedIndex = [];
      var newswappedIndex = [];
      ...
      for (var i = 0; i < this.previousRankings.length; i++) {
        for (var j = 0; j < topCountryToRatio.length; j++) {
          // get country name
          countryName = topCountryToRatio[j].key.replace(/ /g, ""); // found a swap
          ...
          if (topCountryToRatio[j].value > 0 && this.previousRankings[i] === countryName) {
            ...
            d3.select('#row' + countryName).style("position", "absolute").style("top", i * 10 + "%").transition().delay(50).style("top", j * 10 + "%"); // update ratio label

            d3.select("#ratio" + countryName).remove();
            var ratioDiv = d3.select("#mainDiv" + countryName).append("div").attr("width", "90px").attr("height", "20px").attr("id", "ratio" + countryName).style("padding", "2px").append("text").attr("x", "40px").attr("y", "10px").style('color', "#000080").style("font-size", "12px").style("font-weight", "bold").attr("text-anchor", "center") // centering doesn't work
      ...
      for (var i = 0; i < topCountryToRatio.length; i++) {
        // get country name
        countryName = topCountryToRatio[i].key.replace(/ /g, ""); // Remove old elements

        if (!oldswappedIndex.includes(i)) {
          d3.select('#row' + this.previousRankings[i]).style("opacity", 1.0).transition().delay(50).style("opacity", 0.0);
          d3.select('#row' + this.previousRankings[i]).remove();
        } // Add new elements
...
},{"d3":"UzF0"}],"AGkO":[function(require,module,exports) {
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var summaryChartCountry =
/*#__PURE__*/
function () {
  function summaryChartCountry(data, columns) {
    _classCallCheck(this, summaryChartCountry);

    this.data = data;
    this.columns = columns;
  }

  _createClass(summaryChartCountry, [{
    key: "initData",
    value: function initData() {
      var obj = d3.nest().key(function (d) {
        return d.Team;
      }).key(function (d) {
        return d.Year;
    ...
    key: "createChart",
    value: function createChart(country) {
      var result = this.initData();
      console.log("here's the result of init:", result);
      var columns = this.columns;
      d3.select("#table_wrap").attr("width", 100).attr("height", 100);
      var table = d3.select("#country_table").attr("width", 100).attr("height", 100);
      var thead = table.append("thead");
      var tbody = table.append("tbody"); // append the header row

      thead.append("tr").selectAll("th").data(columns).enter().append("th").text(function (columnNames) {
        return columnNames;
      ...
      var rows = tbody.selectAll("tr").data(flattened).enter().append("tr");
      ...
      }).enter().append("td") // .attr("style", "font-family: 'Lato'")
      .html(function (d) {
        return d.value;
      });
    }
  ...
  return summaryChartCountry;
}();

;
module.exports = summaryChartCountry;
},{"d3":"UzF0"}],"epB2":[function(require,module,exports) {
var d3 = require('d3');
...
var yearOptions = ["2000", "2004", "2008", "2012", "2016"]; // Include local JS files:
...
function initializeYearOptions() {
  ...
  rightYear.onclick = function () {
    updateCurrYear("right");
  }; // initialize year label


  document.getElementById("year-window").innerHTML = yearOptions[currYearIndex]; // add in question mark

  d3.select("#question-mark").append("img").attr("src", "icon-question-mark.png").attr("id", "question-icon").attr("width", 20).attr("height", 20);
...
function initializeDropdowns() {
  ...
  select.addEventListener('change', function () {
    ...
    console.log("currYear is ", currYearIndex); // update ranking based on sport selection
    ...
    bigChartInstance.redraw(bigsvg, entriesBySportByYearMedalCount, currSport, medalsvg); // bigChartInstance.drawChart(bigsvg, entriesBySportByYearMedalCount[currSport.value].values, currSport, medalsvg);
  });
}
...
function initializeData(data) {
  ...
  entriesByCountryThenSport = d3.nest().key(function (d) {
    return d.Team;
  }).key(function (d) {
    return d.Sport;
  }).entries(data);
  entriesByCountry = d3.nest().key(function (d) {
    return d.Team;
  }).entries(data); // const myKeys = Object.keys(entriesByCountry) // returns [ 'keyA', 'keyB', 'keyC' ];
  //
  // myKeys.sort((a, b) => {
  //
  ...
  entriesBySport = d3.nest().key(function (d) {
    return d.Sport;
  }).sortKeys(d3.ascending).entries(data); //console.log("figuring out medal count");

  entriesBySportByYearMedalCount = d3.nest().key(function (d) {
    return d.Sport;
  ...
  entriesBySportByYearByCountryRatio = d3.nest().key(function (d) {
    return d.Sport;
  }).key(function (d) {
    return d.Year;
  }).key(function (d) {
    return d.Team;
  }).sortKeys(d3.ascending).entries(data); //console.log("ratio nesting", entriesBySportByYearByCountryRatio);
}
...
function updateCurrYear(direction) {
  ...
  document.getElementById("year-window").innerHTML = yearOptions[currYearIndex]; // update ranks displayed
...
function updateRanking(currSport, currYearIndex) {
  ...
  console.log("yearsData", yearsData); // get all the data for all the countries across a given year (currYear)
  ...
  var athleteNames = [];
  var countryToRatio = []; // Within each country: for each athlete row, count the number of
  ...
  if (countries == null) {
    ...
    countries.values.forEach(function (d) {
      ...
      var athletes = d.values; // console.log("country name:", countryName);
      ...
      countryToRatio.push(new Object({
        key: countryName,
        value: ratio
      }));
    });
    countryToRatio.sort(function (a, b) {
      return a.value > b.value ? -1 : 1;
    });
    topCountryToRatio = countryToRatio.slice(0, 10); // update the Rankings Header
...
function createRanking(sport) {
  ...
  console.log("**********************"); // console.log(currSportOnly);
...
d3.csv('olympics.csv').then(function (data) {
  ...
  initializeData(data); // createRanking("Swimming");
  ...
  bigChartInstance.drawChart(bigsvg, entriesBySportByYearMedalCount, currSport, medalsvg, entriesBySportThenCountryThenYear, rankRows); //createRanking("Archery");
}); // You can load JSON files directly via require.
// Note this does not add a network request, it adds
// the data directly to your JavaScript bundle.
// const exampleData = require('./example-data.json');
},{"d3":"UzF0","underscore":"h15N","./bigChart":"FthO","./rankRows":"V6gp","./summaryChartCountry":"AGkO"}]},{},["epB2"], null)
//# sourceMappingURL=https://uw-cse442-wi20.github.io/FP-olympians/main.885a2def.js.map